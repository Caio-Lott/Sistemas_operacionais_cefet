...
Compilar com gcc -Wall base.c -o b -lpthread && ./b

Nesta tarefa, você deve resolver o problema de deadlock no jantar dos filósofos de duas maneiras:
i) atacando a condição de posse e espera (os filósofos pegam os dois palitos ou nenhum) e
ii) atacando a condição de espera circular (definindo uma ordem para os filósofos pegarem os pali-
tos).
...
void espera (int n)
{
  // Para causar o DEADLOCK é preciso não ter a espera
